...
const EXIT_CONFIDENCE_VALUES = ['high', 'medium', 'low'];
...
function decodePngImage(base64Image) {
...
function isDarkPricePixel(r, g, b, a) {
    return a > 180 && r < 120 && g < 120 && b < 120;
}
function getColumnPriceExtents(data, width, x, yStart, yEnd) {
    const runs = [];
    let runStart = null;
...
function detectDeterministicExitFromDecodedImage(image, scannerContext) {
    ...
    const inferredDirection = context.direction_hint ??
...
function parseNullableDirection(value) {
    return value === 'Long' || value === 'Short' ? value : null;
}
function parseNullableExitReason(value) {
    return value === 'TP' || value === 'SL' ? value : null;
}
function parseNullablePnLResult(value) {
    return value === 'Win' || value === 'Loss' ? value : null;
}
function parseNullableExitConfidence(value) {
    return typeof value === 'string' && EXIT_CONFIDENCE_VALUES.includes(value)
        ? value
        : null;
}
...
function buildEntryTimeHint(extractedEntryTime) {
...
function formatScannerContext(scannerContext) {
    if (!scannerContext) {
...
function describeImageLabel(label) {
    switch (label) {
        case 'full_chart':
...
function selectImagesByLabels(images, labels) {
...
function hasValidLevelStructure(direction, entry, stop, target) {
...
function normalizeExitConfidence(...values) {
...
function pickMostCommonNumber(...values) {
    const counts = new Map();
    values.forEach(value => {
        if (value !== null) {
            counts.set(value, (counts.get(value) ?? 0) + 1);
...
function chooseConsensusNumber(...values) {
...
function inferDirectionFromLevels(entry, stop, target) {
...
function uniqueNumbers(values) {
    return Array.from(new Set(values.filter((value) => typeof value === 'number' && Number.isFinite(value))));
}
function repairTradeStructure(direction, entry, stop, target, labeledCandidates) {
...
function clearExitOutcome(data) {
...
function sanitizeExtractedTradeData(raw) {
    const warnings = Array.isArray(raw.warnings)
        ? raw.warnings.filter((value) => typeof value === 'string' && value.trim() !== '')
        : [];
    const data = {
...
function sanitizeExactPriceRead(raw) {
    return {
        direction: parseNullableDirection(raw.direction),
        entry_price: parseNullableNumber(raw.entry_price),
        sl_price: parseNullableNumber(raw.sl_price),
...
function sanitizeHeaderIdentityRead(raw) {
    return {
        symbol: normalizeScannedSymbol(parseNullableString(raw.symbol)),
        timeframe_minutes: parseNullableNumber(raw.timeframe_minutes),
    };
}
async function extractHeaderIdentity(images) {
...
symbol, timeframe_minutes`;
    return sanitizeHeaderIdentityRead(await callClaudeJson(systemPrompt, images, 'Read only the instrument ticker/root and the timeframe interval printed immediately beside it in the header of the chart containing the colored risk/reward box.', 250));
}
function buildPriceExtractionPrompt(scannerContext) {
    const hasGeometry = Boolean(scannerContext?.entry_line_ratio != null &&
...
async function extractExactPriceLevels(images, scannerContext) {
...
async function verifyPriceDigits(priceImages, candidate, scannerContext) {
    ...
    const imageContent = priceImages.map(img => ({
        type: 'image',
        source: { type: 'base64', media_type: img.mimeType, data: img.base64Image },
    }));
    const pct = (r) => r != null ? (r * 100).toFixed(1) : 'unknown';
    const response = await anthropic.messages.create({
        model: MODEL,
        max_tokens: 1000,
        temperature: 0,
...
Return ONLY:
{
  ...
  "corrections": string[]
    ...
    try {
        ...
        const changed = parsed.entry_price !== candidate.entry_price ||
...
function applyExactPriceRead(base, exactPriceRead) {
    if (!exactPriceRead) {
        return base;
    }
    ...
    const repairedStructure = repairTradeStructure(pickMostCommonString(exactPriceRead.direction, base.direction), chooseConsensusNumber(exactPriceRead.entry_price, base.entry_price), chooseConsensusNumber(exactPriceRead.sl_price, base.sl_price), chooseConsensusNumber(exactPriceRead.tp_price, base.tp_price), {
...
function applyHeaderIdentityRead(base, headerIdentityRead) {
...
async function callClaudeJson(system, images, userText, maxTokens = 1024) {
    const imageContent = images.map(image => ({
        type: 'image',
        source: {
            type: 'base64',
...
async function extractTradeFacts(images, entryDate, fallbackEntryTime, scannerContext) {
...
- exit_reason must be TP or SL only if you can visibly determine which was touched first.`;
    return sanitizeExtractedTradeData(await callClaudeJson(systemPrompt, images, `Trade date: ${entryDate}. Fallback entry time hint if the x-axis is unclear: ${fallbackEntryTime}. ${formatScannerContext(scannerContext)} Extract the trade facts from this screenshot.`, 1100));
}
async function verifyExitOrder(images, entryDate, baseRead, scannerContext) {
...
async function sanityCheckLevelTouches(images, entryDate, baseRead) {
...
async function humanStyleReview(images, entryDate, fallbackEntryTime, scannerContext) {
...
symbol, direction, entry_price, entry_time, entry_time_confidence, sl_price, tp_price, timeframe_minutes, exit_reason, pnl_result, trade_length_seconds, candle_count, exit_confidence, first_touch_evidence`;
    return sanitizeExtractedTradeData(await callClaudeJson(reviewPrompt, images, `Trade date: ${entryDate}. ${buildEntryTimeHint(null)} Fallback time for the form is ${fallbackEntryTime}, but do not let that override the chart itself. Read this chart the same way a human would fill a journal entry.`, 1200));
}
async function decisiveFinalReview(images, entryDate, fallbackEntryTime, extraction, verification, humanReview, scannerContext) {
...
Return ONLY a raw JSON object with these exact keys:
symbol, direction, entry_price, entry_time, entry_time_confidence, sl_price, tp_price, timeframe_minutes, exit_reason, pnl_result, trade_length_seconds, candle_count, exit_confidence, first_touch_evidence`;
    return sanitizeExtractedTradeData(await callClaudeJson(decisivePrompt, images, `Trade date: ${entryDate}. ${buildEntryTimeHint(extraction.entry_time ?? humanReview.entry_time)} ${formatScannerContext(scannerContext)} Fallback time for the form is ${fallbackEntryTime}, but do not let that override the chart itself. Produce the single best final journal-ready trade analysis from this screenshot.`, 1400));
}
async function directExitRead(images, trade, entryDate) {
...
function resolveExitReason(verification, humanReview, decisiveReview, extraction) {
...
function buildManualReaderBase(extraction, humanReview, fallbackEntryTime) {
    const warnings = [
        ...(extraction.warnings ?? []),
        ...(humanReview.warnings ?? []),
    ];
    const humanStructureValid = hasValidLevelStructure(humanReview.direction, humanReview.entry_price, humanReview.sl_price, humanReview.tp_price);
    const extractionStructureValid = hasValidLevelStructure(extraction.direction, extraction.entry_price, extraction.sl_price, extraction.tp_price);
    const repairedStructure = repairTradeStructure(humanReview.direction ?? extraction.direction, chooseConsensusNumber(humanReview.entry_price, extraction.entry_price), chooseConsensusNumber(humanReview.sl_price, extraction.sl_price), chooseConsensusNumber(humanReview.tp_price, extraction.tp_price), {
...
function finalizeManualReaderResult(baseRead, verification, extraction, humanReview) {
...
function applySanityOverride(result, sanity) {
    if (!sanity) {
        return result;
    }
    const next = { ...result };
...
function hasHighConfidenceExit(source, exitReason) {
...
function applyConservativeExitDecision(result, verification, humanReview, decisiveReview, extraction, sanity) {
    ...
    if (next.entry_price != null && next.sl_price != null && next.tp_price != null) {
        ...
        if (looksLong && next.direction === 'Short') {
            next.direction = 'Long';
            appendWarning(next.warnings ?? (next.warnings = []), 'Direction corrected to Long: price structure (TP above entry, SL below) contradicted Short.');
        }
        else if (looksShort && next.direction === 'Long') {
            next.direction = 'Short';
            appendWarning(next.warnings ?? (next.warnings = []), 'Direction corrected to Short: price structure (TP below entry, SL above) contradicted Long.');
        }
        ...
        if (!entryIsValid && next.sl_price != null && next.tp_price != null) {
            ...
            if (swappedSlValid) {
                ...
                appendWarning(next.warnings ?? (next.warnings = []), 'SL and TP were swapped — corrected based on direction and entry price.');
            }
        }
    }
...
function buildConsensusTradeAnalysis(extraction, verification, humanReview, decisiveReview, fallbackEntryTime) {
    const warnings = [
        ...(extraction.warnings ?? []),
        ...(humanReview.warnings ?? []),
        ...(decisiveReview.warnings ?? []),
    ];
    const resolvedStructure = repairTradeStructure(pickMostCommonString(decisiveReview.direction, humanReview.direction, extraction.direction), chooseConsensusNumber(decisiveReview.entry_price, humanReview.entry_price, extraction.entry_price), chooseConsensusNumber(decisiveReview.sl_price, humanReview.sl_price, extraction.sl_price), chooseConsensusNumber(decisiveReview.tp_price, humanReview.tp_price, extraction.tp_price), {
    ...
    result.trade_length_seconds = chooseConsensusNumber(durationSource?.trade_length_seconds, verification.trade_length_seconds, humanReview.trade_length_seconds, decisiveReview.trade_length_seconds, extraction.trade_length_seconds);
    result.candle_count = chooseConsensusNumber(durationSource?.candle_count, verification.candle_count, humanReview.candle_count, decisiveReview.candle_count, extraction.candle_count);
...
async function upscaleLabelCrops(images) {
    const LABEL_CROP_NAMES = new Set(['entry-label-focus', 'stop-label-focus', 'target-label-focus']);
    return Promise.all(images.map(async (img) => {
        if (!LABEL_CROP_NAMES.has(img.label))
            return img;
        try {
            const buffer = Buffer.from(img.base64Image, 'base64');
            const metadata = await (0, sharp_1.default)(buffer).metadata();
            const upscaledBuffer = await (0, sharp_1.default)(buffer)
                .resize(Math.round((metadata.width ?? 100) * 2), Math.round((metadata.height ?? 100) * 2), { kernel: 'nearest' })
                .png()
                .toBuffer();
            return {
                ...img,
...
async function analyzeChartImage(base64Image, mimeType, entryDate, entryTime, focusImages = [], scannerContext) {
    ...
    const analysisImages = [
        ...
        ...focusImages.map((image, index) => ({
            ...
            mimeType: VALID_MIME_TYPES.includes(image.mimeType)
                ? image.mimeType
                : 'image/jpeg',
            label: image.label || `focus_${index + 1}`,
        })),
    ];
    const normalizedScannerContext = scannerContext;
    const decodedFullImage = decodePngImage(base64Image);
    const deterministicExit = detectDeterministicExitFromDecodedImage(decodedFullImage, normalizedScannerContext);
    ...
    const [headerIdentityResult, exactPriceResult, extractionResult, humanReviewResult,] = await Promise.allSettled([
    ...
    if (exactPriceRead && (exactPriceRead.entry_price || exactPriceRead.sl_price || exactPriceRead.tp_price)) {
        try {
            const verified = await verifyPriceDigits(upscaledPriceImages, {
            ...
            if (verified.entry_price != null)
                exactPriceRead.entry_price = verified.entry_price;
            if (verified.sl_price != null)
                exactPriceRead.sl_price = verified.sl_price;
            if (verified.tp_price != null)
                exactPriceRead.tp_price = verified.tp_price;
            if (verified.changed) {
    ...
    const extraction = applyHeaderIdentityRead(applyExactPriceRead(extractionSource, exactPriceRead), headerIdentityRead);
    const rawHumanReview = applyHeaderIdentityRead(applyExactPriceRead(humanReviewSource, exactPriceRead), headerIdentityRead);
    ...
    if (normalizedScannerContext?.direction_hint &&
    ...
    const sanityResult = await (async () => {
        try {
            ...
            appendWarning(baseRead.warnings ?? (baseRead.warnings = []), 'Sanity check failed — relying on exit verification result.');
            return null;
        }
    })();
    let sanityCheck = sanityResult;
    let decisiveReview = rawHumanReview;
    try {
        decisiveReview = applyHeaderIdentityRead(applyExactPriceRead(await decisiveFinalReview(extractionImages, entryDate, entryTime, extraction, verification, rawHumanReview, normalizedScannerContext), exactPriceRead), headerIdentityRead);
    }
    catch {
    ...
    const consensus = applyHeaderIdentityRead(applyExactPriceRead(buildConsensusTradeAnalysis(extraction, verification, rawHumanReview, decisiveReview, entryTime), exactPriceRead), headerIdentityRead);
    const fallbackResult = applyHeaderIdentityRead(applyExactPriceRead(finalizeManualReaderResult(baseRead, verification, extraction, rawHumanReview), exactPriceRead), headerIdentityRead);
    const structureSafeResult = hasValidLevelStructure(consensus.direction, consensus.entry_price, consensus.sl_price, consensus.tp_price)
        ? consensus
        : fallbackResult;
    const finalResult = applyConservativeExitDecision(applySanityOverride(structureSafeResult, sanityCheck), verification, rawHumanReview, decisiveReview, extraction, sanityCheck);
...
async function analyzeIndividualTrade(trade) {
...
async function analyzePatterns(trades) {
...
async function generateWeeklyReport(trades, weekStart, weekEnd) {
...
async function generatePsychologyReport(trades, psychLogs) {
    const emotionalBreakdown = trades.reduce((acc, t) => {
        const state = t.emotional_state || 'Unknown';
        if (!acc[state])
            acc[state] = { count: 0, pnl: 0 };
        acc[state].count++;
        acc[state].pnl += t.pnl;
        return acc;
    }, {});
...
async function compareTradeToPlaybook(trade, playbookEntries) {
    const response = await anthropic.messages.create({
        model: MODEL,
        temperature: MODEL_TEMPERATURE,
        max_tokens: 2048,
...
async function answerFlyxaQuestion(question, history = []) {
...
async function analyzeChartAnalyzerImage(base64Image, mimeType, contractSize) {
    const supportedMimeType = VALID_MIME_TYPES.includes(mimeType)
        ? mimeType
        : 'image/jpeg';
    const response = await anthropic.messages.create({
        model: MODEL,
