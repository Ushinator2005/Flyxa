...
// PUT /settings
router.put('/settings', authMiddleware, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const {
      daily_loss_limit,
      max_trades_per_day,
...
// GET /daily-status
router.get('/daily-status', authMiddleware, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const todayPnL = todayTrades.reduce((s: number, t: { pnl: number }) => s + t.pnl, 0);
